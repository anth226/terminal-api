...
export async function highestYields(intrinioApi) {
  ...
  const url = `${process.env.INTRINIO_BASE_PATH}/securities/screen?order_column=trailing_dividend_yield&order_direction=desc&page_size=50&api_key=${process.env.INTRINIO_API_KEY}`;
  const body = {
  "operator": "AND",
  "clauses": [
      {
